...
llm = ChatGoogleGenerativeAI(
    model=MODEL,             # Specify the Google Gemini model variant to use
...
def read_config_json():
    ...
    if not config_path:
        ...
        print(f"⚠️  MCP_CONFIG_PATH not set. Falling back to: {config_path}")
...
async def run_agent():
    ...
    async with AsyncExitStack() as stack:
        ...
        memory = [{"role": "system", "content": SYSTEM_PROMPT}]  # Ajoute le prompt système comme premier message
        ...
        while True:
            ...
            # Ajoute la requête utilisateur à la mémoire
            memory.append({"role": "user", "content": query})

            # Prépare le contexte pour l'agent : prompt système + historique + nouvelle question
            agent_input = {"messages": memory}

            response = await agent.ainvoke(agent_input)

            # Ajoute la réponse de l'agent à la mémoire
